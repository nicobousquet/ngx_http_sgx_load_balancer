...
int process_msg5 (MsgIO *msgio, IAS_Connection *ias, char **sigrl, config_t *config, ra_session_t *session) {
	...
	char unencrypted_data[128] = "Ici des clefs publiques par exemple !";
	...
	EVP_EncryptUpdate(ctx, (unsigned char *) encrypted_data, &encrypted_len, (unsigned char *) unencrypted_data, 128);
	EVP_EncryptFinal_ex(ctx, (unsigned char *) (encrypted_data + encrypted_len), &encrypted_len);
	
	EVP_CIPHER_CTX_free(ctx);

	memcpy(msg5.msg, encrypted_data, 128);
	msg5.id = 0;

	//msgio->send_partial(&id, sizeof(id));
	msgio->send(&msg5, sizeof(msg5));
	printf("<%.128s> sent.\n", msg5.msg);
